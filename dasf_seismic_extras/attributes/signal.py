...
from dasf_seismic_extras.utils.utils import extract_patches, set_time_chunk_overlap
...
class FirstDerivative(Transform):
    ...
    def __lazy_transform(self, X, xndi, xp):
        ...
        return dask_trim_internal(result2, kernel)
...
class TimeGain(Transform):
    def __init__(self, gain_val=1.5):
        super().__init__()
...
class RMS(Transform):
    def __init__(self, kernel=(1, 1, 9)):
        super().__init__()
...
class ReflectionIntensity(Transform):
    ...
    def _lazy_transform(self, X, xp):
        ...
        result = X.map_blocks(self.__operation, kernel=self._kernel, xp=xp,
                              dtype=X.dtype, chunks=X.chunksize,
                              meta=xp.array((), dtype=X.dtype))
